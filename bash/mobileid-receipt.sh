...
(echo -n $UCS2; echo -n $MSG_TXT | iconv -f UTF-8 -t UCS2) | hexdump -v  -e '/1  "%_ad#    "' -e '/1    "%02X hex"' -e '/1 " = %03i dec"' -e '/1 " = %03o oct"' -e '/1 " = _%c\_\n"'
  (echo -n $UCS2; echo -n $MSG_TXT | iconv -f UTF-8 -t UCS2) | openssl rsautl -encrypt -inkey $PUB_CERT -out $SOAP_REQ.msg -certin > /dev/null 2>&1
